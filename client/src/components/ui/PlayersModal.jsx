      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-ink-black/80" onClick={onClose}>
          <motion.div 
            ...
            className="max-w-md w-full flex flex-col items-start max-h-[90vh] drop-shadow-2xl"
          >
            {/* Folder Tab */}
            <div className="bg-[#d6b87e] border-2 border-b-0 border-[#1c2431] px-6 py-2 rounded-t-[8px] shadow-[inset_0_4px_0_#1c2431] relative z-10 translate-y-[2px] ml-2 flex items-center gap-2">
              <h3 className="text-xl font-typewriter font-bold text-[#1c2431] uppercase tracking-widest">
                Team Roster
              </h3>
              <span className="text-sm font-mono text-[#1c2431] font-bold px-2 border border-[#1c2431] rounded-full bg-white/50">
                {roomState.players?.length} / {roomState.maxPlayers}
              </span>
            </div>
            
            {/* Folder Body */}
            <div className="bg-[#d6b87e] border-2 border-[#1c2431] w-full p-2 relative z-20 flex flex-col min-h-0">
              {/* Paper Document */}
              <div className="bg-white border-2 border-[#1c2431] flex flex-col min-h-0 relative">
                <button onClick={onClose} className="absolute -top-10 right-0 text-white hover:text-red-400 text-3xl font-bold leading-none">&times;</button>
